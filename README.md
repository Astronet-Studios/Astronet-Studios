...
- Optional Dropbox Sign e-signature dispatch for contracts with webhook-based signed status updates.
...
- To enable e-sign contracts, set `DROPBOX_SIGN_API_KEY` (and optionally `DROPBOX_SIGN_TEST_MODE=1`) then configure Dropbox Sign webhook URL to `https://your-domain/api/webhooks/dropbox-sign`.
